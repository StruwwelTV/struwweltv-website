...
export async function GET() {
  ...
  if (!clientId || !clientSecret) {
    return NextResponse.json(
      { error: "Twitch environment variables are missing." },
      { status: 500 },
    );
  }
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      throw new Error(`Twitch token ${tokenResponse.status}: ${await tokenResponse.text()}`);
    }
    ...
    if (!user) {
      return NextResponse.json({ error: "Twitch channel not found." }, { status: 404 });
    }
    ...
    const [streamResult, clipsResult, videosResult] = await Promise.all([
      ...
      helix(`/clips?broadcaster_id=${user.id}&first=20&started_at=${encodeURIComponent(startedAt)}`, clientId, token),
    ...
    console.error(error);
    return NextResponse.json(
      { error: "Twitch data could not be loaded." },
      { status: 502 },
    );
  }
}
