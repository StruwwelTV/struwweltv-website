...
export function AdminControlCenter() {
  ...
  return <main className={styles.shell}><div className={styles.wrap}>
    ...
    <section className={styles.hero}><span className={styles.eyebrow}>STRUWWELTV // CONTROL CENTER</span><h1>Dein Chaos.<br/>Deine Regeln.</h1><p>Streamplan, Website-Texte, Setup und Community-Links direkt verwalten – ohne GitHub und ohne neuen Deploy.</p></section>
    <div className={styles.grid}><nav className={styles.nav}>{(["schedule","website","setup","community"] as Tab[]).map(item=><button key={item} className={`${styles.navButton} ${tab===item?styles.navButtonActive:""}`} onClick={()=>{setTab(item);setMessage("")}}>{item==="schedule"?"Streamplan":item==="website"?"Website":item==="setup"?"Setup":"Community"}</button>)}</nav>
